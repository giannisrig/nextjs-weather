        ...
        const fetchData = async () => {
            ...
            const data = await Promise.all(
                ...
                predefinedLocations.map( async (location, index) => {
                    ...
                    return { key: index, locationData: locationData, weatherData: weatherData, name: location };
                })
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {!loading ? (
                weatherLocationData.map((currentData) => (
                    <WeatherCard key={currentData.key} weatherData={currentData.weatherData} locationData={currentData.locationData} />
